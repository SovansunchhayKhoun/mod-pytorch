...
class Module:
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        super().__setattr__('_static_layer_resources', [])
        super().__setattr__('_count_layers', None)
    ...
    def get_layer_count(self):
        return self._modules.items().__len__()
    
    def set_layer_resource(self, config: List):
        self._static_layer_resources = config
    
    def get_layer_resource(self):
        return self._static_layer_resources
    
    def allocate_layer_resource(self, config: List):
        """Set resource according to user predefined config"""
        for i, (_, module) in enumerate(self._modules.items()):
            if config[i] == 1:
                if torch.cuda.is_available():
                    module = module.cuda()
                else:
                    raise RuntimeError(f'Cannot allocate {module} to cuda')
            else:
                module = module.cpu()

    def register_allocate_layer_resource(self):
        """Allocate resources based on config or randomly if not set"""
        if len(self.get_layer_resource()) == 0:
            layer_resource = []
            for _ in range(self.get_layer_count()):
                layer_resource.append(random.randint(0, 1))
            print(f"Resources allocated randomly: {layer_resource}")
            self.set_layer_resource(layer_resource)
            self.allocate_layer_resource(layer_resource)
            return layer_resource
        else:
            print(f"Resource allocated to defined layer resource: {self.get_layer_resource()}")
            self.allocate_layer_resource(self.get_layer_resource())
            return self.get_layer_resource()
    ...
    def forward_pre_hook(self, input, output):
        if not isinstance(input, torch.nn.modules.loss._Loss):
            print("Forward hook called (internal)")
            layer_resource = self.register_allocate_layer_resource()
            print(f"Layer resource: {layer_resource}")

            layer_device = next(input.parameters()).device
            print(f"Layer device: {layer_device}")

            tensor_tuple = output[0]
            output = tensor_tuple.clone().detach() # convert from tuple to tensor

            output = output.to(layer_device) # change output to layer device

            print(f"Output device: {output.device}")
            output = (output, ) # convert from tuple to tensor
        return output

    def _wrapped_call_impl(self, *args, **kwargs):
        self.register_forward_pre_hook(self.forward_pre_hook) # Register forward pre hook
    ...
    def _call_impl(self, *args, **kwargs):
        ...
        try:
            ...
            if _global_forward_pre_hooks or self._forward_pre_hooks:
                print("🚀 ~ _global_forward_pre_hooks or self._forward_pre_hooks::")
                for hook_id, hook in (
                    ...
                    *self._forward_pre_hooks.items(),
                    ...
                    if hook_id in self._forward_pre_hooks_with_kwargs:
                        ...
                        print(f"_call_impl else Args: {args[0][:5]}:")
            ...
            if full_backward_hooks or backward_pre_hooks:
                print(f"if full_backward_hooks or backward_pre_hooks:")
                bw_hook = hooks.BackwardHook(self, full_backward_hooks, backward_pre_hooks)
                args = bw_hook.setup_input_hook(args)

            result = forward_call(*args, **kwargs)
            if _global_forward_hooks or self._forward_hooks:
                print(f"_global_forward_hooks or self._forward_hooks")
                for hook_id, hook in (
            ...
            if non_full_backward_hooks:
                print(f"non_full_backward_hooks")
                var = result
